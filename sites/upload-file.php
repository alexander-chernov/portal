...
if ($_FILES['site_image']['name']) {
    $uploaddir = '../admin/images/sites/';
    $file = $uploaddir . 'pre' . basename($_FILES['site_image']['name']);
    if (move_uploaded_file($_FILES['site_image']['tmp_name'], $file)) {
        if (exif_imagetype($file) == 1) {
            $file2 = $uploaddir . time() . md5(basename($_FILES['site_image']['name'])) . '.gif';
        }
        if (exif_imagetype($file) == 2) {
            $file2 = $uploaddir . time() . md5(basename($_FILES['site_image']['name'])) . '.jpg';
        }
        if (exif_imagetype($file) == 3) {
            $file2 = $uploaddir . time() . md5(basename($_FILES['site_image']['name'])) . '.png';
        }
