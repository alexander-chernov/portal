    ...
    public function OrganizationAddresses($id) {
        if ($id != 0) {
            ...
            try {
                ...
                GROUP BY k_cfa_address
                ORDER BY k_cfa_id ASC');
                $queue->execute(array(":id" => $id));
                $result = $queue->fetchAll(PDO::FETCH_ASSOC);
                /*
                 * SELECT DISTINCT kcfa.*, kshn.*, ks.*, mb.fid, kcfd.*
                  FROM k_catalog_firms_addresses AS kcfa
                  LEFT JOIN k_streets_house_nums AS kshn ON (kshn.k_shn_id = kcfa.k_cfa_address)
                  LEFT JOIN k_streets AS ks ON (ks.k_s_id = kshn.k_shn_street_id)
                  LEFT JOIN map_buildings AS mb ON (mb.k_shn_id = kshn.k_shn_id)
                  ...
                  WHERE k_cfa_parent=3759
                  GROUP BY k_cfa_address
                  ORDER BY k_cfa_id ASC
                 */

                /*
                 * DROP TABLE IF EXISTS `k_catalog_firms_m`,`k_catalog_firms_addresses_m`,
                  `k_catalog_firms_days_m`,`k_catalog_firms_hours_m`,`k_catalog_firms_parents_m`,
                  `k_catalog_firms_phones_m`,`k_catalog_firms_week_m`
                 */


                /*
                 *  CREATE  TABLE  `k_kedr`.`k_catalog_firms_m` (  `k_cf_id` bigint( 20  )  unsigned NOT  NULL  AUTO_INCREMENT ,
                  ...
                  INSERT INTO `k_kedr`.`k_catalog_firms_m`
                  SELECT k_cf_id,k_cf_name,k_cf_watches,k_cf_email,k_cf_site,c_cf_user FROM `k_kedr`.`k_catalog_firms`;
                 */

                /*
                 *  CREATE  TABLE  `k_kedr`.`k_catalog_firms_addresses_m` (  `k_cfa_id` bigint( 20  )  unsigned NOT  NULL  AUTO_INCREMENT ,
                  ...
                  SET SQL_MODE='NO_AUTO_VALUE_ON_ZERO';
                  INSERT INTO `k_kedr`.`k_catalog_firms_addresses_m` SELECT * FROM `k_kedr`.`k_catalog_firms_addresses`;
                 */

                /*
                 * 
                  CREATE  TABLE  `k_kedr`.`k_catalog_firms_days_m` (  `k_cfh_id` bigint( 20  )  unsigned NOT  NULL  AUTO_INCREMENT ,
                  ...
                  SET SQL_MODE='NO_AUTO_VALUE_ON_ZERO';
                  INSERT INTO `k_kedr`.`k_catalog_firms_days_m` SELECT * FROM `k_kedr`.`k_catalog_firms_days`;
                 */

                /*
                 *  CREATE  TABLE  `k_kedr`.`k_catalog_firms_hours_m` (  `k_cfd_id` bigint( 20  )  unsigned NOT  NULL  AUTO_INCREMENT ,
                  ...
                  SET SQL_MODE='NO_AUTO_VALUE_ON_ZERO';
                  INSERT INTO `k_kedr`.`k_catalog_firms_hours_m` SELECT * FROM `k_kedr`.`k_catalog_firms_hours`;
                 */

                /*
                 *  CREATE  TABLE  `k_kedr`.`k_catalog_firms_parents_m` (  `k_cfp_id` bigint( 20  )  unsigned NOT  NULL  AUTO_INCREMENT ,
                  ...
                  SET SQL_MODE='NO_AUTO_VALUE_ON_ZERO';
                  INSERT INTO `k_kedr`.`k_catalog_firms_parents_m` SELECT * FROM `k_kedr`.`k_catalog_firms_parents`;
                 */

                /*
                 *  CREATE  TABLE  `k_kedr`.`k_catalog_firms_phones_m` (  `k_cfp_id` bigint( 20  )  unsigned NOT  NULL  AUTO_INCREMENT ,
                  `k_cfp_phone` varchar( 50  )  NOT  NULL ,
                  `k_cfp_type` tinyint( 2  )  unsigned NOT  NULL DEFAULT  '1' ,
                  `k_cfp_parent` bigint( 20  )  unsigned NOT  NULL ,
                  PRIMARY  KEY (  `k_cfp_id`  )  ) ENGINE  =  MEMORY  DEFAULT CHARSET  = utf8;
                  SET SQL_MODE='NO_AUTO_VALUE_ON_ZERO';
                  INSERT INTO `k_kedr`.`k_catalog_firms_phones_m` SELECT * FROM `k_kedr`.`k_catalog_firms_phones`;
                 */

                /*
                 *  CREATE  TABLE  `k_kedr`.`k_catalog_firms_week_m` (  `k_cfw_id` tinyint( 2  )  unsigned NOT  NULL  AUTO_INCREMENT ,
                  `k_cfw_name` varchar( 3  )  NOT  NULL ,
                  PRIMARY  KEY (  `k_cfw_id`  )  ) ENGINE  =  MEMORY  DEFAULT CHARSET  = utf8;
                  SET SQL_MODE='NO_AUTO_VALUE_ON_ZERO';
                  INSERT INTO `k_kedr`.`k_catalog_firms_week_m` SELECT * FROM `k_kedr`.`k_catalog_firms_week`;
                 */

                //5338
                $queue_0 = $mysql->prepare('SELECT * FROM k_catalog_firms_days AS kcfd
                    LEFT JOIN k_catalog_firms_week AS kcfw ON (kcfw.k_cfw_id = kcfd.k_cfh_day)
                    WHERE k_cfh_parent=:id
                    ORDER BY k_cfw_id ASC');
                //37276
                $queue_1 = $mysql->prepare('SELECT * FROM k_catalog_firms_hours AS kcfd
                ...
                foreach ($result as $value) {
                    ...
                    $queue_0->execute(array(":id" => $value['k_cfa_id']));
